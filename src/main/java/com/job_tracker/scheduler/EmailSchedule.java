...
public class EmailSchedule {
    ...
    public void createPendingEmail(){
        try {
            List<ReminderEntity> reminderList = reminderRepository.findReminderWithoutEmail(OffsetDateTime.now(ZoneOffset.UTC));

            log.info("Found {} reminders without email", reminderList.size());

            reminderList
                    .forEach(createEmailForReminder::createEmailForReminder);
